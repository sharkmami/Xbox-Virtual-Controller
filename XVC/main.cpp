...
unsigned char calcularGatillo(std::string action) {
    ...
    // Si está mapeado al Mouse (Analógico)
    if (vk == 901 || vk == 902) {
        float pos = obtenerPosicionMouse(vk);
        float finalVal = 0.0f;

        if (action == "RT") {
            // RT se activa cuando el valor es positivo (Mouse arriba o derecha)
            if (pos > 0) finalVal = pos;
        }
        else if (action == "LT") {
            // LT se activa cuando el valor es negativo (Mouse abajo o izquierda)
            if (pos < 0) finalVal = -pos;
        }
    ...
    // Si está mapeado a Tecla o Botón de Mouse (Digital)
...
void motorEmulacion() {
    ...
    while (true) {
        if (emulacionActiva) {
            ...
            // --- GATILLOS (Lógica de Subir/Bajar Mouse) ---
...
int main() {
    ...
    while (seleccion != 5) {
        ...
        std::cout << " 3. Mapear Boton/Mouse" << std::endl;
