...
Instancia del Bus: El código utiliza vigem_alloc() para establecer un puente con el bus de dispositivos virtuales.

Abstracción de Hardware: Mediante vigem_target_x360_alloc(), el sistema operativo reconoce el software como un mando de Xbox 360 físico.

Lógica de Inyección: Los datos se procesan y se envían mediante una estructura XUSB_REPORT cada 2ms. El driver recibe este reporte y lo inyecta en el sistema.
...
Para evitar movimientos bruscos o el "jitter" del mouse, el motor integra una función de Interpolación Lineal (Lerp).
 Lo más importante es que este comportamiento es totalmente configurable por el usuario.

Ajuste Dinámico: Puedes cambiar el smoothing_factor (0.01 a 1.0) desde el panel de control o en el Mapping.ini.

Control de Respuesta: Un valor bajo (0.05) ofrece una fluidez cinematográfica ideal para simuladores, mientras que un valor alto (1.0) desactiva el suavizado para una respuesta instantánea y pura.

📦 Instalación
Instala el driver ViGEmBus. (https://github.com/ViGEm/ViGEmBus/releases)

Ejecuta XVC++.exe

¡Configura tu mapeo y domina el juego! 🦈🚀
